function h = sfrviewer(obj, varargin)
    ...
    %
    %    The SFRVIEVER supports both "singleEvent', 'doubleEvent', and
    %    'singleMarker' annotation structures. See the ANNOTATIONSTRUCT
    %    method for details.
    %    
    ...
    %
    %    The SFRVIEWER does not filter any of the data and decimates the
    ...
    %
    %    <ANNOTATIONS>
    %    If one or more annotation structures are provided, each structure
    %    will have generate a button on the SFRVIEWER UI. Clicking on an
    %    annotation button toggles the viewer between: 1) No view, 2)View
    %    annotation time, 3)View annotation time + annotation value, and 4)
    %    View annotation value.
    %
    %    <PAGING BETWEEN ANNOTATIONS>
    %    If any annotation structures are viewed, they can be selected as
    %    event-page source and the user can page between annotations of the
    %    selected source using the event-forward and event-backward
    %    buttons. A vertical dotted line indicates the aligned event in the
    %    viewer.
    %
